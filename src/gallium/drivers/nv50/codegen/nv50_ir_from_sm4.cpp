...
Converter::inspectDeclaration(const sm4_dcl& dcl)
{
   ...
   case SM4_OPCODE_CUSTOMDATA:
      info.immd.bufferSize = dcl.num * 4;
      info.immd.buffer = (uint32_t *)MALLOC(info.immd.bufferSize);
      memcpy(info.immd.buffer, dcl.data, info.immd.bufferSize);
      break;
...
Converter::handleSAMPLE(operation opr, Value *dst0[4])
{
   ...
   Value *arg[4], *src0[4];
   Value *val;
   int tR = insn->ops[2].indices[0].disp;
   int tS = insn->ops[3].indices[0].disp;
   TexInstruction::Target tgt = resourceType[tR][shadow[tS]];
